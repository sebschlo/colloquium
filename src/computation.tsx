        ...
        useGSAP(() => {
            gsap.to(modelViewerRef.current, {
                scrollTrigger: {
                    ...
                    scrub: true,
                    onEnter: () => {
                        (modelViewerRef.current as any).play({ repetitions: 1 });
                    },
                    onUpdate: (self) => {
                        const progress = self.progress;
                        const rotation = progress * 120; // Adjust rotation as needed
                        (modelViewerRef.current as any).setAttribute('camera-orbit', `${rotation}deg 20deg 3000m`);
                    }
                }
            });
            ...
            if (modelViewerRef.current) {
                (modelViewerRef.current as any).addEventListener('click', (event: { clientX: any; clientY: any; }) => {
                    const hotspotId = (modelViewerRef.current as any).surfaceFromPoint(event.clientX, event.clientY);
                    if (hotspotId) {
